...
pub struct LrActivityRow {
    pub batch: Box<str>,
    /// The link community this stratum measures. Strata ARE communities:
    /// every edge contributes both orientations, so the statistic is
    /// symmetric in the pair and no direction may be read off a row.
    pub community: i32,
    pub ligand: Box<str>,
...
pub struct StratumEntry {
    pub batch: Box<str>,
    /// The link community this stratum measures (a real community id).
    pub community: i32,
    /// Each participating pair once, no direction claimed.
