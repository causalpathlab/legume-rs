//! Bayesian Hierarchical Clustering over K link communities.
//!
...
//! empirical marginal `bg[g] = (Σ_k T_k,g) / (Σ_k S_k)`. The prior expects
//! every cluster to look like `bg`; the BF measures deviation from that
//! baseline. Housekeeping-shaped clusters merge cheaply; clusters with
//! distinct structure stay separate.
//!
...
//! `log_BF ≈ −n · KL(p_i ‖ p_j)`, where n is the merged edge count and KL is
...
//!
//! Runs over K clusters (not E edges). K²·M per merge, K·K²·M overall;
...
pub struct BhcMerge {
    ...
    pub log_bf: f64,
    /// Total edge count under this node (Σ over children).
    pub n_edges: i32,
}
...
struct NodeStats {
    ...
    f_cache: f64,
    /// Public id (original community id for leaves, K .. 2K−2 for internal nodes).
    id: i32,
}
...
/// number of non-empty communities). Empty communities are silently skipped and
...
pub fn bhc_merge(stats: &LinkCommunityStats, gamma: f64) -> Vec<BhcMerge> {
    ...
    for c in 0..k {
        if stats.edge_count[c] == 0 {
            continue;
        }
        let row = &stats.gene_sum[c * m..(c + 1) * m];
        for (dst, &src) in bg.iter_mut().zip(row.iter()) {
            *dst += src;
        }
        total_mass += stats.size_sum[c];
    }
    ...
    // Rescale (T, S) so effective sample size S_eff = edge_count. Preserves proportions
    // T_g / S; puts log BF magnitudes in an interpretable range (see module docstring).
    ...
    for c in 0..k {
        ...
        let s_raw = stats.size_sum[c];
        let n_eff = stats.edge_count[c] as f64;
        let scale = if s_raw > 0.0 { n_eff / s_raw } else { 0.0 };
        let t: Vec<f64> = stats.gene_sum[c * m..(c + 1) * m]
            .iter()
            .map(|&x| x * scale)
            .collect();
        ...
        arena.push(NodeStats {
            t_gene: t,
            s_size: n_eff,
            n_edges: stats.edge_count[c] as i32,
            f_cache: f,
            id: c as i32,
        });
    ...
    while active.len() >= 2 {
        ...
        let new_n = arena[ai].n_edges + arena[aj].n_edges;
...
///
/// Returns dense consensus labels indexed by original community id (length `k`).
/// Communities that were empty (`edge_count == 0` during `bhc_merge` and therefore
/// never appear in any merge) get label `-1`.
///
