...
RUST_LOG=info pinto topic temp7.h5 -c temp/cells.parquet --coord-column-names x_centroid,y_centroid -o temp7 --preload-data -t 10 -v
...
## plot(readLines("temp7.log_likelihood.gz"),type="l")

.pq <- arrow::read_parquet("temp7.latent.parquet")[,-1]

## hist(apply(t(.pq), 2, which.max))

.pairs <- as.data.table(arrow::read_parquet("temp7.coord_pairs.parquet"))
.pairs[, k:= apply(t(.pq), 2, which.max)]

ggplot(.pairs, aes(left_x_centroid, left_y_centroid, colour=as.factor(k))) +
    theme_void() +
    facet_wrap(~k, nrow=2) +
