...
|---|---|---|---|
| bge phase 1 (`graph-embedding-util/src/fit/hier/`) | host `HierParams`: μ_m + r_g; `frozen_rows`, `base_row()`; hand-written gradient, host RowAdagrad | `apply()` skips μ/r for frozen genes, trains `b_g` | **refused until phase 1 runs on candle.** Phase 1 is the one engine without autograd, so a residual there means a second hand-written gradient plus a per-step recompose of every anchored row on the host — inside a 1000-epoch loop that is already most of a bge run. No host/candle mixing inside a phase: the port comes first, then the residual is the masked composition |
...
3. **Phase 1 of bge on candle.** Tables as `Var`s, the two-level softmax as gathers + matmuls + row softmax, one `backward()`, the candle `RowAdagrad` the fne engine already has; the partition, unit table and step plan stay host indexing (ids only, no per-step host arithmetic). The multi-track offsets, support rule and ridge carry over as tensor ops. Parity oracle: `step_tests.rs`'s finite-difference and hand-computed references (`fixture`, `total_loss`, `fixture_tracks`, `total_loss_tracks`) and the planted-program train tests. The host `step.rs`/`params.rs`/optimizer go in the same change. Then LoRA on bge is the masked composition, and phase 1 runs on the GPU beside phase 2.
   - Structured alternative once the port is in: a module-level residual `μ_m = μ₀_m + a_m·V` (genes move with their module, `M·r` parameters). Decide gene-level vs module-level on the four-arm result, not before.
4. ~~Same in fne/simba.~~
