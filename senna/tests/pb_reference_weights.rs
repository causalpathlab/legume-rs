...
use senna::pb_reference::{exclude_carried, weights_for, ReferenceInput, COLUMN_PREFIX};
...
fn new_cells_weigh_one_and_carried_columns_carry_their_counts() {
    ...
    let w = weights_for(&r.cell_counts, &names(5, 3)).expect("well-formed layout");
...
fn carried_columns_are_held_out_of_the_per_cell_outputs() {
    ...
    assert_eq!(
        exclude_carried(Some(&r), 8, None),
        Some(vec![0, 1, 2, 3, 4])
    );

    // QC already dropped cells 1 and 3; the carried columns go too.
    assert_eq!(
        exclude_carried(Some(&r), 8, Some(vec![0, 2, 4, 5, 6, 7])),
        Some(vec![0, 2, 4]),
    );

    // No reference at all leaves the mask exactly as QC left it.
    assert_eq!(exclude_carried(None, 8, Some(vec![0, 2])), Some(vec![0, 2]));
    assert_eq!(exclude_carried(None, 8, None), None);
}
...
fn re_emitting_conserves_the_cells_behind_each_column() {
    ...
    let counts = cell_counts_from(&cell_to_pb, 2, Some(&weight)).expect("matched lengths");
    ...
    assert_eq!(
        cell_counts_from(&cell_to_pb, 2, None).expect("no weights"),
        vec![3.0, 4.0]
    );

    // A wrong-length weight vector means the caller built it in a different
    // column space; silently mis-weighting the tail is the decay above.
    let err = cell_counts_from(&cell_to_pb, 2, Some(&[1.0, 2.0])).expect_err("length mismatch");
    assert!(err.to_string().contains("different column space"), "{err}");
}
