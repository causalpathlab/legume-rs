//! Shared imports for `senna gem`.
//!
//! Follows the crate convention: re-export `candle_core` through
//! `candle_util` so the subcommand modules can do
//! `use crate::gem::common::*;` and then `use candle_core::…` without
//! listing candle-core as a direct dep in `senna/Cargo.toml`. That keeps
//! the `cuda` / `metal` feature graph propagating through a single candle
//! entry point (candle-util), the same way the rest of senna and pinto do.
