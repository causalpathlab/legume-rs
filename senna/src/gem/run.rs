...
fn run_gem_genes_bge(
    ...
    // cell while `cell_embedding.parquet` dropped some, and `senna lineage`
    ...
    // `senna lineage --root-from-gem` as an informed backbone. The `dag_` prefix keeps
    // them distinct from `senna lineage`'s own `{out}.pseudotime.parquet` (Slingshot),
