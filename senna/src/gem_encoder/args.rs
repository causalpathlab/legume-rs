...
pub struct RuntimeArgs {
    ...
    #[arg(
        ...
        long_help = "Compute device.\n\
                     `cuda` / `metal` require the matching cargo feature (`cargo install senna --features cuda`);\n\
...
/// CLI arguments for `senna gem-encoder`.
///
...
pub struct GemEncoderArgs {
    #[arg(
        ...
        long_help = "Gene-level count matrices, given positionally — space-separated,\n\
                     so shell globs work: `senna gem-encoder out/*_genes.zarr.zip`.\n\
    ...
    pub fn genes(&self) -> anyhow::Result<&[Box<str>]> {
        anyhow::ensure!(
            ...
            "no gene matrices given — pass them positionally, \
             e.g. `senna gem-encoder out/*_genes.zarr.zip -o out/gme`"
        );
        Ok(&self.genes_pos)
    }
