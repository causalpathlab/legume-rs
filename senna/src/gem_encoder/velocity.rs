...
//!   still exists and is still used by `senna gem`; only this caller is gone.
//!
...
/// same choice `senna gem` makes for its operator velocity.
///
/// # What is given up
///
...
pub fn write_velocity_tables(
    ...
    // shape and space as before, so `senna lineage` is unaffected.
    let h = alpha.dim(1)?;
