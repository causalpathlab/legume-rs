//! `senna annotate-gem --mode enrichment` — marker-set annotation of a
//! `senna gem-encoder` / `gem-topic` topic model, routed through the topic
...
//! # What it reads
//!
//! `senna annotate-gem` deliberately has no run manifest, so the four tables are read
//! by prefix from a `senna gem-encoder` run:
//!
//! | file | shape | role |
//! |---|---|---|
...
pub fn run(prefix: &str, out: &str, track: Track, args: &AnnotateGemArgs) -> Result<()> {
    ...
    let config = AnnotateConfig {
        ...
        // this command's `--num-perm` is one knob for "how much calibration", and both
        ...
        // error bar. `boot_num_draws` has no CLI flag, so it keeps the library
...
/// The `senna gem-encoder` tables enrichment mode annotates, already reconciled
...
fn required(prefix: &str, suffix: &str) -> Result<String> {
    ...
    anyhow::ensure!(
        ...
        "--mode enrichment needs {path}, which is not there.\n\
         ...
         .pb_latent.parquet come from a `senna gem-encoder` (`gem-topic`) run — point \
         `-f/--from` at that run's `-o` prefix.\n\
         A `senna gem` embedding run writes none of them; annotate one of those with \
         `--mode projection`."
    );
    Ok(path)
}
...
fn align_gene_axis(
    ...
    anyhow::ensure!(
        ...
        "{} of {} genes in {dict_path} have no row in {pb_path} (e.g. {}{}). The two tables must \
         come from the same `senna gem-encoder` run — a stale `pb_gene.parquet` left beside a newer \
         dictionary is the usual cause. Re-run `senna gem-encoder` at this prefix.",
        missing.len(),
        dict_genes.len(),
        missing
            .iter()
        ...
        if !(0.5..=2.0).contains(&mean_sum) {
            warn!(
                "exp() of {path} has mean COLUMN sum {mean_sum:.3}, not ~1 — this file does not \
                 look like log β. `senna gem-encoder` writes log_softmax over genes, so each \
    ...
    match stated {
        ...
        None => warn!(
            "exp() of {path} has mean row sum {mean_sum:.3}, not ~1 — this file does not look \
             ...
             `senna gem-encoder` to get a stamped prefix. Annotation continues on exp() as \
             written."
        ),
    }
    theta
