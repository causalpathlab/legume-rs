//! Entry point for `senna annotate-gem` — marker-set cell-type annotation of a
//! `senna gem` run.
//!
//! A thin front-end over the shared, model-agnostic **term-ORA** core
...
//! `senna annotate-by-projection`, and calls the same core.
//!
//! The two are still separate for one reason: this one discovers its inputs by
//! PREFIX (`{from}.feature_embedding.parquet`, …) while `annotate-by-projection`
//! resolves them through [`crate::run_manifest`]. That split is a leftover from
//! when gem lived in another crate with no manifest to write. Once gem emits
//! `run.senna.json`, everything here except the spliced/velocity track selection
//! becomes redundant with the manifest-driven path, and this module should fold
//! into it rather than keep a second copy of the same statistic.
//!
...
//! space, and β does not share one with θ. See [`crate::gem::marker_embedding`] for the
...
//! single-pass path for the TOPIC models `senna gem-encoder` / `gem-topic` fit,
...
pub enum Mode {
    ...
    /// (`senna gem`). Today's behaviour, unchanged.
    Projection,
    /// Marker-set over-representation across the topic dictionary, carried to
    /// cells through θ (`senna gem-encoder` / `gem-topic`).
    Enrichment,
}
...
pub struct AnnotateGemArgs {
    #[arg(
        long,
        short = 'f',
        help = "gem output prefix (the `-o` value passed to `senna gem`)"
    )]
    pub from: Box<str>,
    ...
    #[arg(
        ...
        long_help = "How the marker panel becomes a cell-type call.\n\
                     ...
                     \n\
                     `projection` is for a `senna gem` EMBEDDING run.\n\
                     ...
                     writes {out}.{track}.*\n\
                     \n\
                     `enrichment` is for a `senna gem-encoder` / `gem-topic` TOPIC-MODEL run.\n\
                     It asks, per factor,\n\
    ...
    #[arg(
        ...
        long_help = "Refuse to annotate on a marker panel the embedding mostly never saw.\n\
                     \n\
                     `senna gem` writes only its TRAINED feature rows,\n\
                     ...
                     `senna gem --markers <the same file>`."
    )]
...
pub fn run_annotate_gem(args: &AnnotateGemArgs) -> Result<()> {
    ...
    if mode == Mode::Enrichment {
        ...
        info!("senna annotate-gem --mode enrichment complete (prefix '{out}')");
        return Ok(());
    }
        ...
        if Path::new(&gene).exists() && Path::new(&cell).exists() {
            ...
            warn!(
                "velocity track skipped: missing {gene} and/or {cell} \
                 (run `senna gem` on spliced+unspliced counts to produce them)"
            );
    ...
    info!("senna annotate-gem complete (prefix '{out}')");
    Ok(())
}
