...
fn main() -> anyhow::Result<()> {
    ...
    match &cli.commands {
        Commands::Svd(args) => {
            ...
            let nsamp = data_vec.partition_columns_to_groups(
                &proj_kn,
                Some(args.sort_dim),
                args.down_sample,
            )?;
        ...
        Commands::Topic(args) => {
            ...
            let batch_nm =
                batch_dn.map(|x| x.posterior_mean().transpose().clone() * &aggregate_rows);
