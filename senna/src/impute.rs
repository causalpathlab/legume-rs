...
pub fn impute_model(args: &ImputeArgs) -> anyhow::Result<()> {
    ...
    info!(
        "Step 1/4: projecting new data through encoder (predict → {})",
        predict_prefix
    );
    ...
    let theta_new_path = format!("{}.latent.parquet", predict_prefix);
    ...
    info!(
        "  θ_new: {} cells × {} topics; θ_ref: {} cells × {} topics",
        n_new, k_new, n_ref, k_ref
    );
    ...
    while col_lb < n_ref {
        ...
        for c_local in 0..csc.ncols() {
            ...
            for (&row_id, &v) in col.row_indices().iter().zip(col.values().iter()) {
                for &(new_id, w) in consumers.iter() {
    ...
    info!(
        "Wrote imputed {}× {} matrix to {}",
        n_new, g_ref, imputed_path
    );
