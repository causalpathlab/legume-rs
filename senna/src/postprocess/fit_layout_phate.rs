...
pub struct LayoutPhateArgs {
    ...
    #[arg(
        ...
        long_help = "Rigid 2D rotation of the PHATE layout.\n\
                     ...
                     Otherwise a fresh principal graph is fit in memory from `outputs.latent`,\n\
                     using --root-cell or --root-node. No artifacts are written in that case.\n\
                     Run `senna pseudotime` first for non-default principal-graph hyperparameters."
    )]
    orient_by_root: bool,
