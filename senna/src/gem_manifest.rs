//! `{out}.gem.json` — WHICH gem-family program produced a prefix, and what its tables
//! mean.
//!
//! # Why this exists
//!
//! The gem-family steps write parquet tables under a shared prefix, and the
...
//! — `{out}.cell_embedding.parquet` is a Euclidean embedding from `senna gem` and
//! a topic membership from `senna gem-encoder` — so a consumer that guesses gets
...
//! [`detect_reporting`].
//!
//! What they do with the answer differs, and correctly so. `senna annotate-gem` picks
//! its whole scorer from it, because projection and enrichment are different
//! statistics over different files. `senna lineage` only caveats its `--markers`
...
pub enum RunKind {
    /// `senna gem` — an embedding run. Cells and genes are placed in one
    /// Euclidean space, so nearest-centroid is meaningful.
    Embedding,
    /// `senna gem-encoder` / `gem-topic` — a topic model. The estimands are `β`
