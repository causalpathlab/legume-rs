...
pub fn fit_principal_graph(
    ...
    let mut y = kmeans_centroids(z, k, args.kmeans_max_iter).0;
...
pub fn pairwise_sqdist_rows_to_rows(a: &DMatrix<f32>, b: &DMatrix<f32>) -> DMatrix<f32> {
    let n = a.nrows();
    let k = b.nrows();
    let d = a.ncols();
...
pub fn mst_from_sqdist(dist_kk: &DMatrix<f32>) -> (Vec<(usize, usize)>, Vec<f32>) {
    let k = dist_kk.nrows();
    if k <= 1 {
