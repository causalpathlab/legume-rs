...
mod tests {
    ...
    #[test]
    fn rough_init_recovers_planted_blocks_at_finest() {
        let sketch = planted_sketch(40, 10, 5, 21);
        let d = sketch.nrows();
        let targets = vec![5usize];
        let knn = FeatureKnnContext::from_sketch(&sketch, 12).unwrap();
        let ml = compute_multilevel_feature_coarsening(&sketch, &targets, &knn).unwrap();
        let fc = &ml.levels[0];
        let truth: Vec<usize> = (0..d).map(|f| f / 40).collect();
        let mut purity_total = 0usize;
        for group in &fc.coarse_to_fine {
            let mut counts: HashMap<usize, usize> = HashMap::default();
            for &f in group {
                *counts.entry(truth[f]).or_default() += 1;
            }
            purity_total += counts.values().copied().max().unwrap_or(0);
        }
        let purity = purity_total as f32 / d as f32;
        // Rough init only — refinement polishes; 0.75 is a generous lower bound.
        assert!(purity >= 0.75, "init purity too low: {purity:.3}");
    }
