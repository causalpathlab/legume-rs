...
/// Target genes per abundance stratum in the de-trended LRT selection null. The LRT
/// scales with a gene's counts (Fisher information), so calling the null WITHIN
/// abundance bins keeps abundant genes from crowding out rare-compartment markers;
/// ~2000 keeps each bin's χ² fit well-determined.
const LRT_STRATUM_TARGET: usize = 2000;
/// Upper bound on abundance strata (a floor on genes-per-bin for the null fit).
const LRT_MAX_STRATA: usize = 24;
...
/// Result of [`select_features_by_lrt`]: per-gene (gene-id order) LRT statistic,
/// pseudobulk detection count, and the live/null call. `live` is the selected
/// training set for the data-driven (`--n-hvg 0`) branch's refit.
...
/// Per-gene LRT scan over **every** gene — the data-driven feature-SELECTION
/// statistic for the `--n-hvg 0` branch. After a broad Pass 1 fixes `θ_pb`, each
/// gene is re-solved on the count-scale pseudobulk view and scored by the
/// likelihood ratio `D_null − D_fit` — the dispersion-aware "does `θ` explain this
/// gene at all" statistic. (`‖β‖²` is invalid here: a re-solved gene has no random
/// init for its null to sit at — see [`FeatureProjectionConfig::null_fdr`] — which
/// is exactly why a norm² null call fails to separate signal on dense data.) The
/// same [`chi2_null_call`] gate the held-out projection uses then calls each gene
/// live/null at `null_fdr`; the caller keeps the live genes and refits (Pass 2).
///
/// `θ` is fixed, so genes are independent — the solve runs in parallel (rayon), like
/// [`project_held_out_features`].
pub(crate) fn select_features_by_lrt(
    pb: &StackedPb,
    h: usize,
    backend_row_to_gene: &[u32],
    backend_unspliced_rows: &[bool],
    ridge: f64,
    null_fdr: f32,
) -> FeatureLrtScan {
    let n_genes = backend_row_to_gene
        .iter()
        .map(|&g| g as usize + 1)
        .max()
        .unwrap_or(0);
    // Bucket each gene's backend rows once. The LRT is scored on the SPLICED rows (the
    // identity profile — matches the held-out β solve); the spliced split is done
    // per-gene inside the parallel solve (keeping it off the serial path), with a
    // fallback to all rows for a gene that has only unspliced rows.
    let mut gene_rows: Vec<Vec<usize>> = vec![Vec::new(); n_genes];
    for (row, &g) in backend_row_to_gene.iter().enumerate() {
    ...
    let scored: Vec<(f32, f32, u32, f32)> = (0..n_genes)
        ...
        .map(|g| {
            let all = &gene_rows[g];
            let spliced: Vec<usize> = all
                .iter()
                .copied()
                .filter(|&r| !backend_unspliced_rows[r])
                .collect();
            ...
            let abundance: f32 = edges.iter().map(|&(_, c)| c).sum();
            // Ridge-escalated solve (see `project_held_out_features`): `lrt < 0` flags a
    ...
    // Only detected & converged genes enter the null fit (undetected → `lrt == 0`
    // structurally; `lrt < 0` → diverged solve). The LRT scales with a gene's counts
    // (Fisher information), so a SINGLE global null lets abundant genes crowd the live
    // set — the Erythroid-dominance bias. De-trend by calling within ABUNDANCE STRATA:
    // sort tested genes by total count, split into bins, and run `chi2_null_call` per
    // bin, so each gene competes against its abundance peers. This surfaces
    // rare-compartment markers instead of only abundant ones, while each bin's
    // statistic stays χ²-like so the call remains valid.
    let mut live = vec![false; n_genes];
    if null_fdr > 0.0 && n_genes > 0 {
        let mut tested: Vec<usize> = (0..n_genes)
            .filter(|&i| n_detected[i] > 0 && lrt[i] > 0.0)
            .collect();
        tested.sort_by(|&a, &b| {
            abundance[a]
                .partial_cmp(&abundance[b])
                .unwrap_or(std::cmp::Ordering::Equal)
        });
        let n_test = tested.len();
        let n_strata = (n_test / LRT_STRATUM_TARGET).clamp(1, LRT_MAX_STRATA);
        let mut sum_sigma2 = 0.0;
        for s in 0..n_strata {
            let stratum = &tested[s * n_test / n_strata..(s + 1) * n_test / n_strata];
            let lrt_s: Vec<f64> = stratum.iter().map(|&i| f64::from(lrt[i])).collect();
            let null = chi2_null_call(&lrt_s, h, null_fdr);
            for (k, &i) in stratum.iter().enumerate() {
                live[i] = null.live[k];
            }
            sum_sigma2 += null.sigma2;
        }
        info!(
            "feature-null (LRT select, {n_strata} abundance strata) — mean σ̂²={:.4}; \
             {} / {n_genes} genes live at FDR {} ({} undetected, {} non-converged excluded)",
            sum_sigma2 / n_strata as f64,
...
/// nothing to `M` — no hard gate is needed. Applying a hard `‖β‖²` null gate here
/// as well is actively harmful: by the time a caller reaches this stage its trained
