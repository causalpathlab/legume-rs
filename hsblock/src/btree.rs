...
///
/// Each internal node stores variational parameters (ln_a0, ln_b0) for
/// the Gamma-Poisson conjugate model at that level of the hierarchy.
#[derive(Debug, Clone)]
pub struct BTree {
    depth: usize,
    num_leaves: usize,
    num_nodes: usize,
    /// log(shape) parameter at each node (1-indexed, index 0 unused)
    pub(crate) ln_a0: Vec<f64>,
    /// log(rate) parameter at each node (1-indexed, index 0 unused)
    pub(crate) ln_b0: Vec<f64>,
}

impl BTree {
    /// Create a new binary tree with the given depth.
    ///
    ...
    pub fn new(depth: usize, init_a0: f64, init_b0: f64) -> Self {
        ...
        let ln_a0 = vec![init_a0.ln(); num_nodes + 1];
        let ln_b0 = vec![init_b0.ln(); num_nodes + 1];
    ...
    pub fn node_params(&self, node: usize) -> (f64, f64) {
        ...
        (self.ln_a0[node].exp(), self.ln_b0[node].exp())
    }
    ...
    pub fn node_ln_params(&self, node: usize) -> (f64, f64) {
        debug_assert!(node >= 1 && node <= self.num_nodes);
        (self.ln_a0[node], self.ln_b0[node])
    }
    ...
    pub fn set_node_ln_params(&mut self, node: usize, ln_a0: f64, ln_b0: f64) {
        debug_assert!(node >= 1 && node <= self.num_nodes);
        self.ln_a0[node] = ln_a0;
        self.ln_b0[node] = ln_b0;
    }
...
mod tests {
    ...
    fn test_tree_sizes() {
        ...
        let tree = BTree::new(3, 1.0, 1.0);
        assert_eq!(tree.num_leaves(), 4);
        assert_eq!(tree.num_nodes(), 7);

        let tree = BTree::new(4, 1.0, 1.0);
