...
/// The whole point of the de-dilution, as a pass/fail: **does MYC survive?**
///
/// MYC is the known-positive faba currently misses — its best site
/// (`chr8:127741259`) runs ~4.2% WT vs ~0.8% control, which clears every gate but
/// drowns when the ~91% of covered cells that never convert are left in the
/// denominator.
///
/// Near-circularity is not a concern: the per-cell rate aggregates over every
/// motif C in the annotation, of which MYC contributes ~186 out of the ~10^5 on
/// chr19, so no cell's verdict is meaningfully driven by the gene being called.
#[test]
#[ignore = "needs the full rep1 BAMs; run with --ignored"]
fn myc_survives_de_dilution_but_not_the_pooled_arm() -> anyhow::Result<()> {
    use crate::editing::cell_activity::scan;
    use crate::editing::pipeline::{
        find_all_conversion_sites, ConversionParams, EditTestLevel, MixtureWeightMode,
    };
    use crate::editing::sifter::{M6aContrast, ModificationType};
    ...
    // chr19 + the MYC locus, subset from rep1 and kept with the rest of the
    // paper data. chr19 alone carries the per-cell statistic; MYC is on chr8, so
    // the call it enables is out-of-sample.
    let d = format!("{}/work/paper-faba/data/bam", std::env::var("HOME")?);
    let wt_bam = format!("{d}/wt_chr19_myc.bam");
    let mut_bam = format!("{d}/mut_chr19_myc.bam");
    let gtf = format!("{d}/chr19_myc.gtf");
    let fasta = "/data3/ypp/ft_qtl/ref/Homo_sapiens_assembly38.fasta";
    for f in [&wt_bam, &mut_bam, &gtf, &fasta.to_string()] {
        if !Path::new(f).exists() {
            eprintln!("skipping: {f} not found");
            return Ok(());
        }
    }

    type PerBam = std::sync::Arc<crate::editing::cell_activity::CompetentCells>;
    let mk = |cells: Option<PerBam>| ConversionParams {
        genome_file: fasta.into(),
        wt_bam_files: vec![wt_bam.as_str().into()],
        mut_bam_files: vec![mut_bam.as_str().into()],
        gene_barcode_tag: "GX".into(),
        cell_barcode_tag: "CB".into(),
        ...
        mixture_weight_mode: MixtureWeightMode::Posterior,
        mixture_prior_alpha: 1.0,
        mixture_prior_beta: 1.0,
        umi_tag: Some("UB".into()),
        site_min_cells: 10,
        test_level: EditTestLevel::Site,
        competent_cells: cells.clone(),
    };

    let gff = GffRecordMap::from(gtf.as_str())?;
    let base = mk(None);

    let wt = scan::scan_cell_activity(&gff, &base, &base.wt_bam_files[0], "WT")?;
    let ct: Vec<CellActivity> =
        scan::scan_cell_activity(&gff, &base, &base.mut_bam_files[0], "MUT")?
            .into_values()
            .collect();
    let call = call_competent_cells(&wt, &ct, &NullCallOpts::default());
    println!(
        "cell QC: kept {}/{} ({:.1}%)  rejected/control {:.2}",
        call.n_selected(),
        call.n_scored,
        100.0 * call.kept_frac(),
        call.rejected_over_control()
    );
    assert!(call.n_selected() > 0, "cell QC kept nothing");

    // MYC's Ensembl id -- the gene key is the id, not the symbol.
    const MYC: &str = "ENSG00000136997";
    let hits = |d: &crate::editing::pipeline::DiscoveredSites, gene: &str| -> usize {
        d.selected
            .iter()
            .filter(|kv| format!("{:?}", kv.key()).contains(gene))
            .map(|kv| kv.value().len())
            .sum()
    };
    let total = |d: &crate::editing::pipeline::DiscoveredSites| -> usize {
        d.selected.iter().map(|kv| kv.value().len()).sum()
    };
    let pooled = find_all_conversion_sites(&gff, &base, None)?;
    // Keyed by the signal BAM, as the per-library sets are.
    let mut by_bam = crate::editing::cell_activity::CompetentCells::default();
    by_bam.insert(wt_bam.as_str().into(), std::sync::Arc::new(call.selected));
    let dediluted = find_all_conversion_sites(&gff, &mk(Some(std::sync::Arc::new(by_bam))), None)?;
    let (a, b) = (hits(&pooled, MYC), hits(&dediluted, MYC));
    println!(
        "selected sites overall: pooled {}, de-diluted {}",
        total(&pooled),
        total(&dediluted)
    );
    println!("MYC selected sites: pooled arm {a}, de-diluted arm {b}");
    assert!(
        b > 0,
        "MYC did not survive de-dilution (pooled {a}, de-diluted {b})"
    );
    Ok(())
}
